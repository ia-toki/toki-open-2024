...
long long min_weight(int N, int M, int K,
                     ...
                     std::vector<int> P) {
    // this reduction attempt to K=1 won't work;
    // we may find a "shortcut" through the dummy node
    ...
    for (int i = 0 ; i < M ;) {
        ...
        while (j < M && get<2>(sortedEdges[j]) == get<2>(sortedEdges[i])) {
            ...
            minsSeen = min(minsSeen, query(1, tin[root], tout[root], tin[u], tin[u]).mins + w);
            minsSeen = min(minsSeen, query(1, tin[root], tout[root], tin[v], tin[v]).mins + w);
            j++;
        }

        if (minsSeen < INF) {
            int uu = finds(source);
            update(1, tin[root], tout[root], tin[uu], tout[uu], minsSeen);
        }
