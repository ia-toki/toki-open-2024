...
long long min_risk(int N, int M, int L, std::vector<int> U, std::vector<int> V,
    ...
    while (!dq.empty()) {
        ...
        for (int nxt : adj[cur]) {
            // only try smallest and smallest that is larger
            // might be slow? the binary search is causing the complexity to be O(LM log M)
            vector<int> nextButtons = {buttons[nxt][0]};
            auto nextLarger = upper_bound(buttons[nxt].begin(), buttons[nxt].end(), button);
            if (nextLarger != buttons[nxt].end()) {
                nextButtons.push_back(*nextLarger);
            }

            for (int nextButton : nextButtons) {
